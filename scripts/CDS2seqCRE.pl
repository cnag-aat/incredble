...
my $nt_out =
  new Bio::SeqIO( -format => 'Fasta', '-file' => ">$newfile.cds.fa" );
my $aa_out =
  new Bio::SeqIO( -format => 'Fasta', '-file' => ">$newfile.pep.fa" );
$newfile =~ s/\.gff//g;
...
foreach my $cdsr ( sort keys %cds_recs ) {
    ...
    foreach my $cds (@sortedexons) {
        $ntseq .= SeqOp::get_seq_BioDBFasta( $db, $sid, $cds->[3], $cds->[4],
            $cds->[6] );
    }
    my $description = $p{$cdsr};
    if ( $ntseq !~ /^ATG.*(TAA|TGA|TAG)$/ || $sortedexons[0]->[7] ) {
        $description .= ";partial_cds";
    }
    $cdsr =~ s/\.mrna//;
    ...
    my $protcdsobj =
      $cdsobj->translate( undef, undef, $sortedexons[0]->[7], $code );
