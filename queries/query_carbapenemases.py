...
for samp in sample_qs:
    ...
    #    num+=s.num_genes
    scaffolds_qs = Scaffold.objects.all().filter(sample=samp.id).annotate(num_genes=Count('gene_set', filter=(Q(gene_set__rgi='Yes') & Q(gene_set__rgi_set__best_hit_aro='VIM-1'))))
    for s in scaffolds_qs:
        num+=s.num_genes
    resistance=0
